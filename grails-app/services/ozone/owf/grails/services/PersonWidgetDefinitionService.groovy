...
//           }
      ...
      JSON.parse(params.toDelete).each { w ->
        ...
        if (pwd != null && pwd.size() > 0) {
          ...
          pwd[0].dashboardWidgetStates.each{ dws ->
              dws.dashboard?.state?.remove(dws)
          }
          pwd[0].delete();
        }
      }
...
//            groupWidgetsToTagsMap[widgetDef.widgetGuid] << ['name': group.name, 'visible': true, 'position': -1]
      ...
      if(params?.tags) {
          for(tag in JSON.parse(params?.tags)) {
              ...
              PersonWidgetDefinition.findAllByTag(tag).each {
	...
	def delete (params) {
		...
		if (personWidgetDefinition != null) {
			...
			def widDefD = WidgetDefinition.findByWidgetGuid(params.guid)
    	 	...
    	 	try
			{
				//Manually remove DWS from their dashboard parent.
				//Otherwise, we will get a re-save during cascade.
				personWidgetDefinition.dashboardWidgetStates.each{
					it.dashboard?.state?.remove(it)
				}
