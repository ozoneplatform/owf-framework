...
class AccountService {
    ...
    private def detachDashboardsForUser(person)
    {
        //DashboardWidgetState objects have two parents. We'll get the re-saved by cascade error.
        person.dashboards.each{dashboard ->
...
//    def unAssignUserFromAllGroups(params) {
//      def person
//      if (params.person) {
...
//    }
    ...
    def deleteUser(params){
        ...
        try {
            ...
            def dashboards = Dashboard.withCriteria {
                eq('createdBy',person)
            }
            ...
            //search through all dashboards for dashboards which have reference to the user being deleted
            dashboards = Dashboard.withCriteria {
                eq('editedBy',person)
            }
