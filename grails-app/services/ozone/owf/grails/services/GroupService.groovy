...
class GroupService {
    ...
    def removeGroupWidgetsFromPerson(group,person) {

        def groupWidgets = domainMappingService.getMappedObjects(group,RelationshipType.owns,WidgetDefinition.TYPE)

        if (!groupWidgets.isEmpty() && person) {
            def widgetInstances = DashboardWidgetState.withCriteria {
                dashboard {
                    eq('user',person)
                }
                inList('widgetGuid',groupWidgets.widgetGuid)
                //        not {
            ...
            println "widgetInstances:$widgetInstances"
            widgetInstances*.delete(flush:true)

        }
    }

    def removeAllGroupWidgets(group) {
