...
<%
    ...
    if (query && query.toLowerCase().contains("csp-debug=true")) {
        response.addHeader("Content-Security-Policy-Report-Only", "default-src 'self';")
    }
 %>
<html>
    <head>
        ...
        <!-- ** CSS ** -->
        <owf:themeStylesheet/>
        <owf:bootstrapStylesheet/>

        <!-- initialize ozone configuration from server -->
        <owf:configScript/>

        <!-- modernizr-->
        <owf:vendor src="modernizr/modernizr-2.6.2.js"/>
        ...
        <owf:javascript src="owf-server.js"/>
        <!-- include our server bundle, in dev mode a full list of js includes will appear -->
        ...
        <script>
            ...
            ;(function(Ozone, Ext, jQuery) {
                ...
                if (!Ozone.config.showAnimations) {
                    ...
                    ss.setAttribute('href', '${request.contextPath}/themes/a_default.theme/css/animations.css');
        ...
        <owf:javascript src="marketplace/marketplaceAPI.js"/>
    </head>
