...
// bug in 1.0.4, so copy file here and patch
...
target('default': "Runs a Grails application in Jetty with HTTPS listener") {
    ...
    //set up variables
    keystore = config.keystore ? config.keystore : "${userHome}/.grails/${grailsVersion}/ssl/keystore"
    keystoreFile = new File("${keystore}")
    keyPassword = config.keyPassword? config.keyPassword : "123456"
