...
abstract class AbstractAuditingFilters {
	...
	protected void logLogOnEvent(HttpServletRequest request){		
		...
		if(doCefLogging()){
			CEF cef = CEFFactory.buildBaseCEF(EventTypes.LOGON.getDescription(), "A logon event occured.", 7)
	...
	protected void logLogOffEvent(HttpServletRequest request){		
		...
		if(doCefLogging()){
			CEF cef = CEFFactory.buildBaseCEF(EventTypes.LOGOFF.getDescription(), "A logoff event occured.", 5)
			def fields = ExtensionFactory.getBaseExtensionFields(request, getUserName(), getApplicationVersion(),getHostClassification())
	...
	protected void logSearchEvent(HttpServletRequest request, def model){
		...
		if(doCefLogging()){						
			CEF cef = CEFFactory.buildBaseCEF(EventTypes.OBJ_SEARCH.getDescription(), "A search action was performed.", 5)
			def fields = ExtensionFactory.getBaseExtensionFields(request, getUserName(), getApplicationVersion(), getHostClassification())
		...
		if(doCefLogging()){
		  ...
		  if (request.getContentType()?.toLowerCase()?.startsWith("multipart/form-data"))
		  {
			request.getFileNames().each{
				CEF cef = CEFFactory.buildBaseCEF(EventTypes.IMPORT.getDescription(), "A file was imported.", 7)
				def fields = ExtensionFactory.getBaseExtensionFields(request, getUserName(), getApplicationVersion(),getHostClassification())
		  ...
		  else
		  {
			  ...
			  CEF cef = CEFFactory.buildBaseCEF(EventTypes.IMPORT.getDescription(), "A JSON object was imported.", 7)
			  def fields = ExtensionFactory.getBaseExtensionFields(request, getUserName(), getApplicationVersion(),getHostClassification())
			  fields[Extension.EVENT_TYPE] = EventTypes.IMPORT.getDescription()
	...
	protected void logExportEvent(def request, def response){	
		...
		if(doCefLogging()){
			CEF cef = CEFFactory.buildBaseCEF(EventTypes.EXPORT.getDescription(), "A file was exported.", 7)
			def fields = ExtensionFactory.getBaseExtensionFields(request, getUserName(), getApplicationVersion(),getHostClassification())
