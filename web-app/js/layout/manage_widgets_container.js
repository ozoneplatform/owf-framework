        ...
        this.widgetGrid = Ext.create('Ext.grid.GridPanel', {
            ...
            columns: [
                ...
                },
                {
                    ...
                    cellDisabled: function(record) {
                        return record.data['removed'] || record.get('editable') === false;
                    },
                    renderer : function(value, metaData, record, rowIndex, colIndex, store) {
                ...
                },
                {
                    ...
                    cellDisabled : function(record) {
                        if (record.get('_editable') === false || record.get('editable') === false) {
                            return true;
                        }
                        else {
                            return false;
                    ...
                    onChecked: function(view, record) {
                        view.addRowCls(record.index, "x-item-disabled");
                    },
                    onUnchecked: function(view, record) {
                        view.removeRowCls(record.index, "x-item-disabled");
                    },
                    renderer : function(value, metaData, record, rowIndex, colIndex, store) {
                        var thisColumn = this.columns[colIndex];
                        var cls = [thisColumn._checkBoxCls];
