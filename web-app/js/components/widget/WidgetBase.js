Ext.define('Ozone.components.widget.WidgetBase', {
	...
	overrides : {
		...
		ghostTools: function() {
			var tools = [],
			headerTools = this.header.query('button, tool[hidden=false]');
			...
			if (headerTools.length) {
				Ext.each(headerTools, function(tool) {
					...
					if(tool.isXType('tool', true)) {
							tools.push({
									type: tool.type
							});
					}
					else {
							tools.push(tool.cloneConfig());
					}
				});
			}
			else {
		...
		/***@override private - used for dragging and rendering the icon*/
		ghost: function(cls) {
			var me = this,
			...
			if (!ghostPanel) {
				ghostPanel = Ext.create('Ext.panel.Panel', {
					...
					title: Ext.htmlEncode(me.title),  // force creation of header by setting a title
					icon:me.icon      // to include icon
				});
				me.ghostPanel = ghostPanel;
			}

			var iconCmpId = 'headerIconCmp-'+ ghostPanel.id;
			if(Ext.getCmp(iconCmpId) == undefined){
				var iconCmp = Ext.create('Ext.Component', {
						id: iconCmpId,
						focusable: false,
						renderTpl : ['<img alt="" src="{icon}" class="{cls}-header-icon {iconCls}"/>'],
						renderData: {
								icon:  ghostPanel.icon != null ? ghostPanel.icon : Ext.BLANK_IMAGE_URL,
								cls: ghostPanel.baseCls,
								iconCls: ghostPanel.iconCls,
								orientation: ghostPanel.orientation
						},
						renderSelectors: {
								iconEl: '.' + ghostPanel.baseCls + '-icon'
						},
						icon: ghostPanel.icon,
						iconCls: ghostPanel.iconCls
				});
			...
			if (header) {
				...
				Ext.Array.forEach(header.query('button, tool'), function(tool){
					header.remove(tool);
				});
