                        ...
                        searchChanged: {
                            fn: function (cmp, value) {
                                ...
                                if (value != '') {
                                    ...
                                    if (!(grid.store.findRecord('term', value))) {
                                        grid.store.add({term: value});
                                        cmp.setRawValue('');
                                        this.search();
                                    }
                                }
                                ...
                                itemkeydown: {
                                    fn: function (view, record, dom, index, evt) {
                                        switch (evt.getKey()) {
                                            ...
                                            case evt.ENTER:
                                                var grid = this.down("#tagGrid");
                                                if (!(grid.store.findRecord('tag', record.data.tag))) {
                                                    grid.store.add({tag: record.data.tag});
                                                    this.search();
                                                }
                                                break;
                                            default:
            ...
            for (var i = 0, len = containerWidgetStore.getCount() ; i < len ; i++ ) {
                ...
                if (keepWidgetFlag) {
                    for (var j = 0; j < termStore.getCount(); j++) {
                        var term = termStore.getAt(j).get('term');
                        isFiltered = true;
                ...
                if (keepWidgetFlag) {
                    ...
                    for (var j = 0; j < tagStore.getCount(); j++) {
                        var tag = tagStore.getAt(j).get('tag');
