...
class DashboardServiceTests extends GroovyTestCase {
	...
	protected void tearDown(){
		super.tearDown()
	/*	tearDownRequisiteDbDomains()
			PersonWidgetDefinition.withTransaction { TransactionStatus status ->
	        	PersonWidgetDefinition.list()*.delete()
	        }
	        Person.withTransaction { TransactionStatus status ->
	        	Person.list()*.delete()
	        }
	        WidgetDefinition.withTransaction { TransactionStatus status ->
	        	WidgetDefinition.list()*.delete()
	        }*/
	}
	
	/*private tearDownRequisiteDbDomains(){
			person.save(flush:true)
			Person.withTransaction { TransactionStatus status ->
				def pWidDefList = PersonWidgetDefinition.list()
				Long size = pWidDefList.size()
				for(int i in 0..<size){
					
					PersonWidgetDefinition pWidDef = (PersonWidgetDefinition) pWidDefList[i]
					WidgetDefinition widDef = pWidDef.widgetDefinition
	    			widDef.personWidgetDefinitions.remove(pWidDef)
	    			pWidDef?.delete()
	    		}
	    		person.personWidgetDefinitions.clear()
	    	}
		}
		
		private setUpRequisiteDbDomains(){
			//Generate WidgetDefs for dashboard states...
			
		
			
			WidgetDefinition.withTransaction { TransactionStatus status ->
				widgetDefinitionA1_bulkA.save()
				widgetDefinitionA2_bulkA.save()
				widgetDefinitionA3_bulkA.save()
				widgetDefinitionBCH1_bulkBCD.save()
				widgetDefinitionBCH2_bulkBCD.save()
				widgetDefinitionDG1.save()
				widgetDefinitionDG2.save()
				widgetDefinitionEF1.save()
				widgetDefinitionEF2.save()
			}
			
			def pWidDefA1_bulkA = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionA1_bulkA,
					pwdPosition: 0,
					visible: true
			)
	    	
	    	def pWidDefA2_bulkA = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionA2_bulkA,
					pwdPosition: 1,
					visible: true
			)
	    	
	    	def pWidDefA3_bulkA = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionA3_bulkA,
					pwdPosition: 2,
					visible: true
			)
			
	    	def pWidDefBCH1_bulkBCD = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionBCH1_bulkBCD,
					pwdPosition: 3,
					visible: true
			)
			
	    	def pWidDefBCH2_bulkBCD = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionBCH2_bulkBCD,
					pwdPosition: 4,
					visible: true
			)
			
	    	def pWidDefDG1 = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionDG1,
					pwdPosition: 5,
					visible: true
			)
	    	def pWidDefDG2 = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionDG2,
					pwdPosition: 6,
					visible: true
			)
	    	def pWidDefEF1 = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionEF1,
					pwdPosition: 7,
					visible: true
			)
	    	def pWidDefEF2 = new PersonWidgetDefinition(
					person: person,
					widgetDefinition: widgetDefinitionEF2,
					pwdPosition: 8,
					visible: true
			)
	    
			PersonWidgetDefinition.withTransaction { TransactionStatus status ->
				pWidDefA1_bulkA.save()
				pWidDefA2_bulkA.save()
				pWidDefA3_bulkA.save()
				pWidDefBCH1_bulkBCD.save()
				pWidDefBCH2_bulkBCD.save()
				pWidDefDG1.save()
				pWidDefDG2.save()
				pWidDefEF1.save()
				pWidDefEF2.save()
			...
			person.addToPersonWidgetDefinitions(pWidDefA1_bulkA)
			person.addToPersonWidgetDefinitions(pWidDefA2_bulkA)
			person.addToPersonWidgetDefinitions(pWidDefA3_bulkA)
			person.addToPersonWidgetDefinitions(pWidDefBCH1_bulkBCD)
			person.addToPersonWidgetDefinitions(pWidDefBCH2_bulkBCD)
			person.addToPersonWidgetDefinitions(pWidDefDG1)
			person.addToPersonWidgetDefinitions(pWidDefDG2)
			person.addToPersonWidgetDefinitions(pWidDefEF1)
			person.addToPersonWidgetDefinitions(pWidDefEF2)
			
			widgetDefinitionA1_bulkA.addToPersonWidgetDefinitions(pWidDefA1_bulkA)
			widgetDefinitionA2_bulkA.addToPersonWidgetDefinitions(pWidDefA2_bulkA)
			widgetDefinitionA3_bulkA.addToPersonWidgetDefinitions(pWidDefA3_bulkA)
			widgetDefinitionBCH1_bulkBCD.addToPersonWidgetDefinitions(pWidDefBCH1_bulkBCD)
			widgetDefinitionBCH2_bulkBCD.addToPersonWidgetDefinitions(pWidDefBCH2_bulkBCD)
			widgetDefinitionDG1.addToPersonWidgetDefinitions(pWidDefDG1)
			widgetDefinitionDG2.addToPersonWidgetDefinitions(pWidDefDG2)
			widgetDefinitionEF1.addToPersonWidgetDefinitions(pWidDefEF1)
			widgetDefinitionEF2.addToPersonWidgetDefinitions(pWidDefEF2)
    ...
    void testCreate() {
		...
		assertEquals 3, result.dashboard.state.size()
    }
	...
	void createDashboardsForBulkTests(){
		...
		assertEquals 3, dashboardA.state.size()
		...
		assertEquals 2, dashboardB.state.size()
		...
		assertEquals 2, dashboardC.state.size()
		...
		assertEquals 2, dashboardD.state.size()
	...
	void testCreateNewDefault()
	{
			...
			assertEquals 2, originalDefaultDashboard.state.size()
			assertEquals 2,   secondDefaultDashboard.state.size()
	}
	...
	void testUpdateDashboardPositionWithLayoutParameter()
	{
		//Create 2 Person Widget Definitions so the state objects match up.
		assertNotNull makePersonWidgetDefinition(WDPP.generateDSTPostParamsB1())
		assertNotNull makePersonWidgetDefinition(WDPP.generateDSTPostParamsB2())
		...
		resultOfCreate = dashboardService.create(generatePostParamsB())
		assertTrue resultOfCreate.success
		...
		assertEquals "12345678-1234-1234-1234-1234567890a1", dashboards[0].guid //Dashboard D-G
		assertEquals "12345678-1234-1234-1234-1234567890a2", dashboards[1].guid //Dashboard E-F
		assertEquals "12345678-1234-1234-1234-1234567890a0", dashboards[2].guid //Dashboard B-H

		resultOfList = dashboardService.list(["layout":"tabbed"])
		dashboards = resultOfList.dashboardList
		assertEquals "12345678-1234-1234-1234-1234567890a1", dashboards[0].guid //Dashboard D-G
		assertEquals "12345678-1234-1234-1234-1234567890a0", dashboards[1].guid //Dashboard B-H
	}

	void testConvertToTabbed()
	{
		def dashboard = buildI()
		dashboardService.convertDashboardLayout(dashboard,"tabbed",false)
		assertEquals("tabbed", dashboard.layout)

		def first = dashboard.state.first()
	...
	void testConvertToTabbedWithBypass()
	{
		//TODO Jeremy and Vicki to consult on this test re: Vicki's bypass changes.

			/*def dashboard = buildI()
						dashboardService.convertDashboardLayout(dashboard,"tabbed",true)
						assertEquals("tabbed", dashboard.layout)
	...
	void testConvertToPortal()
	{
		def dashboard = buildI()
		dashboardService.convertDashboardLayout(dashboard,"portal",false)
		assertEquals("portal", dashboard.layout)

		def columnToCheck = 0
		dashboard.state.each{
			assertFalse(it.active)
			assertFalse(it.collapsed)
			assertEquals(columnToCheck, it.columnPos)
			columnToCheck ++
			if (columnToCheck == 3)
			{
				columnToCheck = 0
	...
	void testConvertToPortalWithBypass()
	{
			def dashboard = buildI()
			dashboardService.convertDashboardLayout(dashboard,"portal",true)
			assertEquals("portal", dashboard.layout)
	...
	void testConvertToDesktop()
	{
		def dashboard = buildI()
		dashboardService.convertDashboardLayout(dashboard,"desktop",false)
		assertEquals("desktop", dashboard.layout)

		def last = dashboard.state.last()
		assertTrue(last.active)

		def position = 0
		dashboard.state.each{
			assertFalse(it.collapsed)
			assertEquals(position, it.x)
			assertEquals(position, it.y)
			assertEquals(position, it.zIndex - 9000)
			if (it != last){
				assertFalse(it.active)
			}
			position += 10
	...
	void testConvertToDesktopWithBypass()
	{
		def dashboard = buildI()
		dashboardService.convertDashboardLayout(dashboard,"desktop",true)
		assertEquals("desktop", dashboard.layout)
	...
	void testConvertToAccordion()
	{
		def dashboard = buildI()
		dashboardService.convertDashboardLayout(dashboard,"accordion",false)

		assertEquals("accordion", dashboard.layout)

		def foundRSNorth = null
		def foundRSSouth = null
		def accordionCount = 0
		def noneCount = 0
		dashboard.state.each{
			if (it.region == "rightSideNorth")
			{
				if (foundRSNorth)
				{
					fail("Duplicate Right Side North entities")
				}
				foundRSNorth = it
			}
			else if (it.region == "rightSideCenter")
			{
				if (foundRSSouth)
				{
					fail("Duplicate Right Side South entities")
				}
				foundRSSouth = it
			}
			else if (it.region == "none")
			{
				noneCount ++
			}
			else if (it.region == "accordion")
			{
				assertTrue(it.collapsed)
				assertEquals(200,it.width)
				accordionCount ++
			}
			else
			{
				fail("Invalid region: " + it.region)
			}
		}
		assertNotNull(foundRSNorth)
		assertNotNull(foundRSSouth)
		assertEquals("12345678-1234-1234-1234-123456789132", foundRSNorth.personWidgetDefinition.widgetDefinition.widgetGuid)
		assertEquals("12345678-1234-1234-1234-123456789142", foundRSSouth.personWidgetDefinition.widgetDefinition.widgetGuid)
		assertEquals(0,noneCount)
		assertEquals(5,accordionCount)
	}



	void testConvertToAccordionWithBypass()
	{

	//TODO Jeremy and Vicki to consult on this test re: Vicki's bypass changes.

	/*	def dashboard = buildI()
			dashboardService.convertDashboardLayout(dashboard,"accordion",true)
