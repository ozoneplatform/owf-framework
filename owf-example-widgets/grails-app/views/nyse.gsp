...
<head>
    ...
    <owf:frameworkJs src="components/focusable/Focusable.js"/>
    <owf:frameworkJs src="components/focusable/FocusableGridPanel.js"/>
    <owf:frameworkJs src="components/layout/SearchBoxLayout.js"/>
    <owf:frameworkJs src="components/form/field/SearchBox.js"/>

    <owf:widgetJs src="StockTimer.js"/>
    ...
    <script type="text/javascript">
        ...
        function init() {
                        ...
                        if (widgetList != null) {
                            ...
                            if (found) {
                                OWF.RPC.getWidgetProxy(widgetId, function (widget) {
                                    if (Ext.isFunction(widget.doPlot)) {
                                        widget.doPlot(JSON.stringify(data));
                                    }
                                });
                            } else {
                                if (_tm) {
            ...
            var NYSEStore = Ext.create('Ext.data.Store', {
                ...
                proxy: {
                    type: 'ajax',
                    url: '${request.contextPath}/static/widgets/data/NYSE.xml',
                    reader: {
                        type: 'xml',
                        record: 'Row'
                    }
                                    ...
                                    function (dest) {
                                        ...
                                        if (dest.length > 0) {
                                            Ext.Array.each(dest, function (datum, index, dataRef) {
                                                ...
                                                var proxy = datum.id;
                                                if (_tm) {
                                                    _tm.reset(widgetId, proxy, symbols);
                                                }
                                            });
                                        }
                                        else {
