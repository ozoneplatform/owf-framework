Ext.define('Ozone.data.OWFTransportProxy', {
    ...
    doRequest: function (operation, callback, scope) {
        ...
        if (this.updateActions && this.updateActions[operation.action]) {
            operation.params.update_action = this.updateActions[operation.action];
        }
        var proxyScope = this;
        ...
        var proxyCallback = function (operation, json) {
            ...
            if (Ext.isFunction(proxyScope.callback)) {
                proxyScope.callback(operation, json);
            }
        };
